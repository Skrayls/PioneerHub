...
- Active milestone: KYC Status Navigator V1 (PR #52 open from `feature/kyc-status-navigator-v1`). The implementation adds the anonymous browser-only route `/kyc-busena`: four fixed KYC-question situations and safe next steps. It uses only local rendering: no Pi sign-in, KYC documents, screenshots, usernames, wallet details, status claims, backend, persistent storage, payments or Mainnet capability. Full local validation, Worker dry-run, Pi-boundary validation, GitHub CI, and Workers Build passed for implementation head `6655246`; CI must pass again for this status-documentation checkpoint before merge.
