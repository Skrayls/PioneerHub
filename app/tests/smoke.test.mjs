...
assert.match(js, /const FRONTEND_BUILD = 'merchant-readiness-desk-v1'/);
...
const shell = await worker.fetch(new Request('https://example.test/?build=merchant-readiness-desk-v1'), {
...
assert.match(shellHtml, /href="\/styles\.css\?v=merchant-readiness-desk-v1"/);
assert.match(shellHtml, /src="\/app\.js\?v=merchant-readiness-desk-v1"/);
...
for (const [route, shellSource, assets] of [
  ...
  for (const asset of assets) assert.match(routeHtml, new RegExp(`(?:href|src)="${asset.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\?v=merchant-readiness-desk-v1"`), `${route} must emit root-relative ${asset}`);
...
assert.match(workerSource, /const FRONTEND_BUILD = "merchant-readiness-desk-v1";/, 'App Inspector build marker must be current');
