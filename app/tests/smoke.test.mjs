...
assert.doesNotMatch(html, /BlackMerchanter|referral_open|minepi\.com\/[^"'\s>]+\?u=/i);
assert.match(html, /App Radar/);
assert.match(html, /href="\/sauga"/);
assert.doesNotMatch(html, /Testnet Payment Lab|id="lab"|Pi Auth diagnostika/i);
assert.doesNotMatch(html, /href="#lab"/);
assert.doesNotMatch(html, /REQUIRES PI DEVELOPER PORTAL CONFIGURATION/);
assert.doesNotMatch(html, /sdk\.minepi\.com\/pi-sdk\.js/);
...
assert.doesNotMatch(js, /Pi\.authenticate|Pi\.init|Pi\.signIn|\/api\/pi\/auth|createPayment/);
...
assert.doesNotMatch(response.headers.get('content-security-policy'), /sdk\.minepi\.com/);
assert.equal(response.headers.get('cache-control'), 'no-cache');

const shell = await worker.fetch(new Request('https://example.test/?build=organic-discovery-readiness-v1'), {
...
assert.match(shellHtml, /href="\/styles\.css\?v=organic-discovery-readiness-v1"/);
...
assert.match(shell.headers.get('content-security-policy') || '', /https:\/\/sdk\.minepi\.com 'nonce-/);
assert.match(shellHtml, /id="pi-support-channel"/);
assert.match(shellHtml, /PALAUKYMO KANALAS/);
assert.match(shellHtml, /PI TESTNET/);
assert.match(shellHtml, /Prisijungti per Pi/);
assert.match(shellHtml, /Palaikyti PioneerHub — 0\.01 Test-Pi/);
assert.match(shellHtml, /const amount = 0\.01, memo = "PioneerHub Support Channel"/);
assert.match(shellHtml, /purpose":"pioneerhub_support_channel/);
assert.match(shellHtml, /scopes = \["username", "payments", "roles", "in_app_notifications"\]/);
...
for (const [route, shellSource, assets] of [
  ...
  for (const asset of assets) assert.match(routeHtml, new RegExp(`(?:href|src)="${asset.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\?v=organic-discovery-readiness-v1"`), `${route} must emit root-relative ${asset}`);
...
assert.match(workerSource, /const FRONTEND_BUILD = "organic-discovery-readiness-v1";/, 'App Inspector build marker must be current');
...
assert.match(paymentChecklistHtml, /primaryScopes = \['username', 'payments'\]/);
assert.match(paymentChecklistHtml, /pi\.authenticate\(\['username', 'payments'\], onIncompletePaymentFound\)/);
assert.equal((paymentChecklistHtml.match(/\.authenticate\(/g) || []).length, 1, 'checklist auth must make one bounded SDK attempt');
...
assert.match(paymentChecklistHtml, /AUTH_PI_INIT_FAILED|AUTH_PI_PRIMARY_FAILED|AUTH_PI_CONSENT_SCOPE_FAILED/);
...
assert.match(paymentChecklistHtml, /AUTH_SCOPE_STRATEGY/);
assert.match(paymentChecklistHtml, /Scope strategy: .*Pi message:/);
...
assert.match(sandboxChecklistHtml, /Pi\.authenticate\(\[\], onIncompletePaymentFound\)/);
assert.match(sandboxChecklistHtml, /const scopes = \[\];/);
assert.doesNotMatch(sandboxChecklistHtml, /\['username', 'payments'\]|\['payments'\]|wallet_address/);
...
assert.match(sandboxChecklistHtml, /revealDiagnostics\(\); setState\('PI PLATFORM APP AUTH BLOCKED'\);/, 'Sandbox empty-scope auth failure must reveal diagnostics before reporting the blocked state');
...
assert.match(sandboxChecklistHtml, /BASE_PI_AUTH_VERIFIED/, 'empty-scope server-verified auth must be identified without granting payment permission');
assert.match(sandboxChecklistHtml, /setState\('BASE PI AUTH VERIFIED'\)/, 'empty-scope auth success must stop before enabling payments');
assert.match(sandboxChecklistHtml, /BASE_AUTH_FAILED_WITH_EMPTY_SCOPES/, 'empty-scope auth failure must provide the final isolation marker');
assert.match(sandboxChecklistHtml, /PI_PLATFORM_APP_AUTH_BLOCKED/, 'empty-scope auth failure must provide the platform-blocked marker');
assert.match(sandboxChecklistHtml, /authorization = session\.authorization; render\('SERVER_AUTH_VERIFICATION_COMPLETE'\); render\('BASE_PI_AUTH_VERIFIED'\);\s*setState\('BASE PI AUTH VERIFIED'\);/, 'base auth must stop after server verification without enabling the payment button');
