...
assert.match(js, /const FRONTEND_BUILD = 'learn-v2'/);
...
const shell = await worker.fetch(new Request('https://example.test/?build=learn-v2'), {
...
assert.match(shellHtml, /styles\.css\?v=learn-v2/);
assert.match(shellHtml, /app\.js\?v=learn-v2/);
assert.match(shellHtml, /Build: learn-v2/);
...
assert.match(workerSource, /const FRONTEND_BUILD = "learn-v2";/, 'Learn product build marker must be current');
