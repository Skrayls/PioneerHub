...
assert.equal((worker.match(/\.authenticate\(/g) || []).length, 3, 'authentication may exist only in isolated diagnostic harnesses');
assert.match(worker, /Pi\.createPayment\(\{ amount, memo, metadata \}, callbacks\)/);
assert.equal((worker.match(/Pi\.createPayment\(/g) || []).length, 2, 'createPayment must exist only in isolated checklist harnesses');
assert.match(worker, /await Pi\.init\(\{ version: "2\.0", sandbox: true \}\)/);
assert.equal((worker.match(/sandbox: true/g) || []).length, 1, 'Sandbox mode must be isolated to one dedicated diagnostic harness');
