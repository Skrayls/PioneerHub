...
assert.equal((worker.match(/\.authenticate\(/g) || []).length, 4, 'authentication may exist only in diagnostic harnesses or the root Support Channel');
assert.match(worker, /Pi\.createPayment\(\{ amount, memo, metadata \}, callbacks\)/);
assert.equal((worker.match(/Pi\.createPayment\(/g) || []).length, 3, 'createPayment must exist only in isolated checklist harnesses or the root Support Channel');
...
assert.doesNotMatch(worker, /isOfficialPiSandboxRootRequest|ancestorOrigins|pi-root-sandbox-control/);
