...
const home = await get('/?build=pi-dual-auth-r11');
assert.equal(home.status, 200, 'homepage must return HTTP 200');
const homeHtml = await home.text();
assert.match(homeHtml, /PioneerHub/);
assert.match(homeHtml, /Build: pi-dual-auth-r11/);
assert.match(homeHtml, /app\.js\?v=pi-dual-auth-r11/);
