...
const home = await get('/?build=pi-signin-oauth-r10');
assert.equal(home.status, 200, 'homepage must return HTTP 200');
const homeHtml = await home.text();
assert.match(homeHtml, /PioneerHub/);
assert.match(homeHtml, /Build: pi-signin-oauth-r10/);
assert.match(homeHtml, /app\.js\?v=pi-signin-oauth-r10/);
