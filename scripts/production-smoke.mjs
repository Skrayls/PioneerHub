...
const home = await get('/?build=auth-username-scope-r8');
assert.equal(home.status, 200, 'homepage must return HTTP 200');
const homeHtml = await home.text();
assert.match(homeHtml, /PioneerHub/);
assert.match(homeHtml, /Build: auth-username-scope-r8/);
assert.match(homeHtml, /app\.js\?v=auth-username-scope-r8/);
...
const css = await get('/styles.css?v=auth-username-scope-r8');
assert.equal(css.status, 200, 'CSS must return HTTP 200');
assert.match(css.headers.get('cache-control') || '', /immutable/);
const js = await get('/app.js?v=auth-username-scope-r8');
