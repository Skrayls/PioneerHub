...
const home = await get('/?build=auth-sdk-head-r5');
assert.equal(home.status, 200, 'homepage must return HTTP 200');
const homeHtml = await home.text();
assert.match(homeHtml, /PioneerHub/);
assert.match(homeHtml, /Build: auth-sdk-head-r5/);
assert.match(homeHtml, /app\.js\?v=auth-sdk-head-r5/);
