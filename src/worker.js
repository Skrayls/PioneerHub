...
<style>
  ...
  * { box-sizing: border-box; } body { margin: 0; min-width: 320px; } main { width: min(100%, 760px); margin: 0 auto; padding: 24px 18px 48px; }
  ...
  #sandbox-state { margin: 18px 0 0; padding: 14px 15px; border-left: 4px solid #6387d3; border-radius: 8px; background: #eef3fb; color: #334a70; line-height: 1.5; } #sandbox-diagnostics { margin-top: 22px; padding: 0 0 14px; border-top: 1px solid #d9e1ed; } #sandbox-diagnostics[hidden] { display: none; } #sandbox-diagnostics-title { margin: 0; padding: 16px 0; color: #29466f; font-size: 1rem; font-weight: 750; } #sandbox-diagnostic-log { margin: 0 0 8px; padding: 0 0 0 22px; color: #516079; font: .78rem/1.55 ui-monospace, SFMono-Regular, Menlo, monospace; overflow-wrap: anywhere; } #sandbox-diagnostic-log li { padding: 5px 0; }
...
(() => {
  ...
  const revealDiagnostics = () => {
    ...
    diagnostics.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
  };
...
function piSupportSdkBootstrap(nonce) {
  return `<script src="https://sdk.minepi.com/pi-sdk.js"></script><script nonce="${nonce}">(() => {
  const boot = window.__PIONEERHUB_PI_SUPPORT_BOOT__ = { staticScript: true, sdkAvailableOnLoad: typeof Pi !== 'undefined', initStarted: false, initComplete: false, initError: '' };
  boot.initStarted = true;
  boot.ready = Promise.resolve().then(() => {
    if (typeof Pi === 'undefined') throw new Error('sdk_missing');
    return Pi.init({ version: "2.0", sandbox: true });
  }).then(() => { boot.initComplete = true; }).catch(error => { boot.initError = error?.message || 'pi_init_failed'; });
})();</script>`;
}

function piSupportChannel(nonce) {
  return `<section id="pi-support-channel" class="section" aria-labelledby="pi-support-title"><style nonce="${nonce}">#pi-support-channel{border:1px solid #cbd5e1;background:#f8fbff}#pi-support-channel .pi-testnet{display:inline-block;padding:.25rem .55rem;border-radius:999px;background:#f5b642;color:#17223b;font-weight:800;font-size:.75rem;letter-spacing:.08em}#pi-support-channel button{margin:.6rem .6rem .6rem 0;padding:.72rem 1rem;font:inherit}#pi-support-channel [hidden]{display:none}#pi-support-log{overflow-wrap:anywhere;font:.78rem/1.5 ui-monospace,monospace}</style><p class="eyebrow">PALAUKYMO KANALAS</p><span class="pi-testnet">PI TESTNET</span><h2 id="pi-support-title">Palaikyk PioneerHub</h2><p>Prisijunk per Pi ir išbandyk PioneerHub palaikymo kanalą.</p><p>0.01 Test-Pi</p><button id="pi-support-auth" type="button" disabled>Prisijungti per Pi</button><button id="pi-support-payment" type="button" disabled>Palaikyti PioneerHub — 0.01 Test-Pi</button><p id="pi-support-state" aria-live="polite">Tai Testnet bandymas. Tikras Pi nenaudojamas.</p><section id="pi-support-diagnostics"><h3>Techninė diagnostika</h3><ol id="pi-support-log" aria-live="polite"></ol></section></section><script nonce="${nonce}">(() => {
  ...
  const amount = ${PI_SUPPORT_CHANNEL_AMOUNT}, memo = ${JSON.stringify(PI_SUPPORT_CHANNEL_MEMO)}, metadata = ${JSON.stringify(PI_SUPPORT_CHANNEL_METADATA)}, scopes = ["username", "payments", "roles", "in_app_notifications"], boot = window.__PIONEERHUB_PI_SUPPORT_BOOT__; let authorization = '', busy = false, incompletePayment = null;
  ...
  render('PI_SDK_SCRIPT_STATIC', String(Boolean(boot?.staticScript))); render('PI_SDK_AVAILABLE_ON_LOAD', String(Boolean(boot?.sdkAvailableOnLoad))); render('PI_INIT_ON_LOAD_STARTED'); render('AUTH_BUTTON_REQUIRES_NO_BOOTSTRAP', 'true');
  if (!boot?.ready) { render('PI_INIT_ON_LOAD_ERROR', 'bootstrap_missing'); fail(); } else boot.ready.then(() => { if (!boot.initComplete) throw new Error(boot.initError || 'pi_init_failed'); render('PI_INIT_ON_LOAD_COMPLETE'); authButton.disabled = false; }).catch(error => { render('PI_INIT_ON_LOAD_ERROR', error?.message || 'pi_init_failed'); fail(); });
  authButton.addEventListener('click', async () => { if (busy || !boot?.initComplete) return; busy = true; authButton.disabled = true; try { render('PI_AUTHENTICATE_STARTED',JSON.stringify(scopes)); const result = await Pi.authenticate(scopes, onIncompletePaymentFound); render('PI_AUTHENTICATE_RESOLVED', JSON.stringify({ accessTokenExists: Boolean(result?.accessToken), userExists: Boolean(result?.user), uidExists: Boolean(result?.user?.uid) })); if (!result?.accessToken || !result?.user?.uid) throw new Error('access_token_or_user_missing'); const response = await fetch('/api/pi/auth',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({accessToken:result.accessToken})}); const session = await response.json().catch(() => ({})); if (!response.ok || !session?.authenticated || typeof session.authorization !== 'string') throw new Error('server_auth_failed'); authorization = session.authorization; state.textContent = 'Prisijungta per Pi Testnet.'; if (!incompletePayment) paymentButton.disabled = false; } catch (error) { render('PI_AUTHENTICATE_ERROR_MESSAGE', error?.message || 'Authentication failed.'); fail(); } finally { busy = false; if (!authorization && boot?.initComplete) authButton.disabled = false; } });
...
export default { async fetch(request, env) {
  ...
  if (isShell) {
    ...
    let html = (await response.text())
      ...
      .replace("</body>", isSignInCallback ? `${piSignInCallbackBootstrap(nonce, version)}</body>` : url.pathname === "/" ? `${piSupportSdkBootstrap(nonce)}${piSupportChannel(nonce)}</body>` : "</body>");
